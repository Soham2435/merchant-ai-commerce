...
begin
  ...
  /*
   * Determine whether the authenticated caller is a merchant
   * member for this merchant.
   *
   * Merchant members use the RPC from the merchant workspace.
   * Non-members are treated as buyer callers and therefore
   * require an explicit buyer spending authorization.
   */
  select exists (
    select 1
  ...
  /*
   * Buyer authorization is required only for non-merchant callers.
   */
  if not is_merchant_member then
  ...
  /*
   * Merchant transaction limit remains a database-level
   * safety boundary for all order creation.
   */
  ...
  begin
    ...
    values (
      p_merchant_id,
      case
        when is_merchant_member then null
        else authenticated_user_id
      end,
      p_idempotency_key,
      request_fingerprint,
      'pending',
...
revoke execute on function public.create_pending_order(uuid, text, jsonb)
from public;

grant execute on function public.create_pending_order(uuid, text, jsonb)
to authenticated;